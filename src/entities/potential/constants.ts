...
export const possibleStatRegexRecord: Record<PossibleStat, RegExp> = {
  "STR %": /^\s*STR\s*:\s*\+(\d+)\s*%\s*$/,
  "DEX %": /^\s*DEX\s*:\s*\+(\d+)\s*%\s*$/,
  "INT %": /^\s*INT\s*:\s*\+(\d+)\s*%\s*$/,
  "LUK %": /^\s*LUK\s*:\s*\+(\d+)\s*%\s*$/,
  ...
  ALL: /^\s*올스탯\s*:\s*\+(\d+)\s*$/,
  AUTO_STEAL: /^\s*공격\s*시\s*(\d+)%\s*확률로\s*오토스틸\s*$/,
};

// 세 개의 옵션 중 최대 한 개만 존재할 수 있는 옵션의 정규식
export const maxOneOptionRegexList = [/쓸만한/, /피격\s*후\s*무적시간/];

// 세 개의 옵션 중 최대 두 개만 존재할 수 있는 옵션
export const maxTwoOptionRegexList = [
